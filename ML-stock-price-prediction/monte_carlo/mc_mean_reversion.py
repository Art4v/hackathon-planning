...
# --- Model: Mean Reversion (Ornstein-Uhlenbeck on returns) ---
def mean_reversion(S0, days, theta=0.01, mu=0.005, sigma=0.085):
    dt      = 1 / 252
    sqrt_dt = math.sqrt(dt)
    prices  = [S0]
    S       = S0
    r       = mu          # initial return starts at the long-run mean
    for _ in range(days):
...
# --- Simulate ---
all_paths    = [mean_reversion(S0, DAYS) for _ in range(N_RUNS)]
...
ax.set_title(
    f'Mean Reversion (Ornstein-Uhlenbeck) — Monte Carlo Simulation\n'
...
# --- Save ---
out_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'mc_mean_reversion.png')
